...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd,const WCHAR* vsFilename,const WCHAR* psFilename)
{
	...
	// 1 because there is already 
	// a TEXCOORD for the texture coordinates using slot 0

	polygonLayout[2].Format = DXGI_FORMAT_R32G32B32_FLOAT; // x,y,z pos vector
	polygonLayout[2].InputSlot = 1;
	polygonLayout[2].AlignedByteOffset = 0;
	polygonLayout[2].InputSlotClass = D3D11_INPUT_PER_INSTANCE_DATA; // indicates that this is a instance data

	// InstanceDataStepRate = 1 indicates that we will draw one instance before stepping
	//forward a unit in the instance data.
