...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	float specularPower, XMFLOAT4 color)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, normalStrength, lightDirection,
		diffuseColor, translation, rotation, scale, cameraPosition, specularColor, specularPower, color);
	if (!result)
	{
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC matrixBufferDesc, lightBufferDesc, transformBufferDesc, cameraBufferDesc, colorBufferDesc;
...
bool ColorShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix,
	XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float normalStrength, XMFLOAT3 lightDirection,
	...
	XMFLOAT4 color)
{
	...
	if (FAILED(result)) { return false; }
	dataPtr4 = (ColorBufferType*)mappedResource.pData;

	dataPtr4->color = color;
